...
class QuickBooks_Status_Report
{
     ...
     *
     */
    public function create($mode, $user = null, $date_from = null, $date_to = null, $fetch_full_record = false, $restrict = [])
    {
        $Driver = $this->_driver;
     ...
     */
    public function status($user = null, $levels = [])
    {
        $Driver = $this->_driver;
        ...
        if (!count($levels)) {
            $levels = [
                60 * 60 * 12 => [ QuickBooks_Status_Report::STATUS_NOTICE, 'Notice: A connection has not been made in %d days, %d hours and %d minutes.' ],
                60 * 60 * 24 => [ QuickBooks_Status_Report::STATUS_CAUTION, 'Caution: A connection has not been made in %d days, %d hours and %d minutes.' ],
                60 * 60 * 36 => [ QuickBooks_Status_Report::STATUS_WARNING, 'Warning! A connection has not been made in %d days, %d hours and %d minutes.' ],
                60 * 60 * 48 => [ QuickBooks_Status_Report::STATUS_DANGER, 'DANGER! A connection has not been made in %d days, %d hours and %d minutes! Contact support to get this issue resolved!' ],
                ];
        }

        if (!isset($levels[0])) {
            $levels[0] = [ QuickBooks_Status_Report::STATUS_OK, 'Status is OK. Last connection made %d days, %d hours, and %d minutes ago.' ];
        }

        if (!isset($levels[-1])) {
            $levels[-1] = [ QuickBooks_Status_Report::STATUS_UNKNOWN, 'Status is unknown.'];
        }
    ...
    public function HTML($mode, $user = null, $date_from = null, $date_to = null, $fetch_full_record = false, $restrict = [], $skip_empties = true)
    {
