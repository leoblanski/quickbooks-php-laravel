...
class QuickBooks_Callbacks
{
    ...
    public static function callAuthenticate($Driver, $callback, $username, $password, &$customauth_company_file, &$customauth_wait_before_next_update, &$customauth_min_run_every_n_seconds)
    {
        ...
        if ($type == QuickBooks_Callbacks::TYPE_OBJECT_METHOD) {			// Object instance method hook
            ...
            $ret = QuickBooks_Callbacks::_callObjectMethod([ $object, $method ], $vars, $err, $which);
        } elseif ($type == QuickBooks_Callbacks::TYPE_FUNCTION) {		// Function hook
            $err = '';
        ...
        } elseif ($type == QuickBooks_Callbacks::TYPE_STATIC_METHOD) {		// Static method hook
            if ($Driver) {
    ...
    protected static function _callObjectMethod($object_and_method, &$vars, &$err, $which = null)
    {
        ...
        if (is_callable([ $object, $method])) {
            $ret = call_user_func_array([ $object, $method ], $vars);
    ...
    protected static function _callStaticMethod($class_and_method, &$vars, &$err, $which = null)
    {
        ...
        if (is_callable([ $class, $method])) {
            $ret = call_user_func_array([ $class, $method ], $vars);
    ...
    protected static function _type(&$callback, $Driver = null, $ticket = null)
    {
        ...
        } elseif (is_object($callback) and $callback instanceof QuickBooks_Hook) {
    ...
    public static function callAPICallback($Driver, $ticket, $callback, $method, $action, $ID, &$err, $qbxml, $qbobject, $qbres)
    {
        ...
        } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_FUNCTION) {		// Function hook
            if ($Driver) {
        ...
        } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_STATIC_METHOD) {		// Static method hook
            if ($Driver) {
     ...
     * @return boolean
     */
    public static function callHook($Driver, &$hooks, $hook, $requestID, $user, $ticket, &$err, $hook_data, $callback_config = [])
    {
        ...
        if (isset($hooks[$hook])) {
            ...
            foreach ($hooks[$hook] as $callback) {
                ...
                $vars = [ $requestID, $user, $hook, &$err, $hook_data, $callback_config ];
                if ($type == QUICKBOOKS_CALLBACKS_TYPE_OBJECT_METHOD) {			// Object instance method hook
                    $object = $callback[0];
                    $method = $callback[1];
                    ...
                    $ret = QuickBooks_Callbacks::_callObjectMethod([ $object, $method ], $vars, $err);
                    //$ret = call_user_func_array( array( $object, $method ), array( $requestID, $user, $hook, &$err, $hook_data, $callback_config) );
                } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_FUNCTION) {		// Function hook
                    if ($Driver) {
                        $Driver->log('Calling hook function: ' . $callback, $ticket, QUICKBOOKS_LOG_VERBOSE);
                    ...
                    //$ret = call_user_func_array( array( $class, $method ), array( $requestID, $user, $hook, &$err, $hook_data, $callback_config) );
                } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_HOOK_INSTANCE) {
                    ...
                    $ret = QuickBooks_Callbacks::_callObjectMethod([ $callback, 'hook' ], $vars, $err);
                } else {
                    return false;
                }
     ...
     *
     */
    public static function callRequestHandler($Driver, &$map, $requestID, $action, $user, $ident, $extra, &$err, $last_action_time, $last_actionident_time, $version = '', $locale = [], $callback_config = [], $qbxml = null)
    {
     ...
     *
     */
    public static function callResponseHandler($Driver, &$map, $requestID, $action, $user, $ident, $extra, &$err, $last_action_time, $last_actionident_time, $xml = '', $qb_identifiers = [], $callback_config = [], $qbxml = null)
    {
     ...
     */
    protected static function _callRequestOrResponseHandler($Driver, &$map, $requestID, $action, $which, $user, $ident, $extra, &$err, $last_action_time, $last_actionident_time, $xml_or_version = '', $qb_identifier_or_locale = [], $callback_config = [], $qbxml = null)
    {
        //print_r($map);
        //print('action: ' . $action . "\n");
        //print('which: ' . $which . "\n");
        ...
        if (is_array($tmp)) {
            if (isset($tmp[$which])) {
                ...
                $vars = [ $requestID, $user, $action, $ident, $extra, &$err, $last_action_time, $last_actionident_time, $xml_or_version, $qb_identifier_or_locale, $callback_config, $qbxml ];
                ...
                if ($type == QUICKBOOKS_CALLBACKS_TYPE_OBJECT_METHOD) {
                    ...
                    return $xml;
                } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_STATIC_METHOD) {
                    $err = '';
                    ...
                    return $xml;
                } elseif ($type == QUICKBOOKS_CALLBACKS_TYPE_FUNCTION) {
                    $err = '';
    ...
    public static function callAuth($Driver, $callback, $username, $password, &$qb_company_file, &$wait_before_next_update, &$min_run_every_n_seconds, &$err)
    {
    ...
    public static function callErrorHandler($Driver, $errmap, $errnum, $errmsg, $user, $requestID, $action, $ident, $extra, &$errerr, $xml, $callback_config)
    {
        ...
        } elseif (isset($errmap[$action])) {
            $callback = $errmap[$action];
        } elseif (isset($errmap['!'])) {		// catch-all error handler		//  and function_exists($this->_onerror['!'])
            $callback = $errmap['!'];
        } elseif (isset($errmap['*'])) {		// catch-all error handler		//  and function_exists($this->_onerror['*'])
            $callback = $errmap['*'];
        }
