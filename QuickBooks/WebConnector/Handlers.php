...
class QuickBooks_WebConnector_Handlers
{
     ...
     */
    public function __construct($dsn_or_conn, $map, $onerror, $hooks, $log_level, $input, $handler_config = [], $driver_config = [], $callback_config = [])
    {
    ...
    protected function _defaults($config)
    {
        ...
        if (!is_array($config['allow_remote_addr'])) {
            $config['allow_remote_addr'] = [ $config['allow_remote_addr'] ];
        }
    ...
    public function authenticate($obj)
    {
        ...
        $hookerr = '';
        $this->_callHook($ticket, QUICKBOOKS_HANDLERS_HOOK_AUTHENTICATE, null, null, null, null, $hookerr, null, [], $hookdata);
        ...
        if (is_array($override_dsn) or strlen($override_dsn)) { 	// Custom autj
            ...
            if (QuickBooks_Callbacks::callAuthenticate($this->_driver, $override_dsn, $obj->strUserName, $obj->strPassword, $customauth_company_file, $customauth_wait_before_next_update, $customauth_min_run_every_n_seconds) and
                ...
                if ($customauth_company_file) {
                    ...
                    $status = $company_file;
                } elseif ($this->_config['qb_company_file']) {
                ...
                $hookerr = '';
                $this->_callHook($ticket, QuickBooks_WebConnector_Handlers::HOOK_LOGINSUCCESS, null, null, null, null, $hookerr, null, [], $hookdata);
                ...
                $hookerr = '';
                $this->_callHook(null, QUICKBOOKS_HANDLERS_HOOK_LOGINFAILURE, null, null, null, null, $hookerr, null, [], $hookdata);
            ...
            if ($ticket = $this->_driver->authLogin($obj->strUserName, $obj->strPassword, $company_file, $wait_before_next_update, $min_run_every_n_seconds)) {
                ...
                $hookerr = '';
                $this->_callHook($ticket, QUICKBOOKS_HANDLERS_HOOK_LOGINSUCCESS, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function sendRequestXML($obj)
    {
        ...
        if ($this->_driver->authCheck($obj->ticket)) {
            ...
            $hookerr = '';
            $this->_callHook($obj->ticket, QUICKBOOKS_HANDLERS_HOOK_SENDREQUESTXML, null, null, null, null, $hookerr, null, [], $hookdata);
                ...
                if (!($requestID = $this->_extractRequestID($xml)) and
                    ...
                    foreach (QuickBooks_Utilities::listActions() as $action) {
                        ...
                        if (false !== strpos($xml, '<' . $request . ' ')) {
                            ...
                            break;
                        } elseif (false !== strpos($xml, '<' . $request . '>')) {
                            //$xml = str_replace('<' . $request . '>', '<' . $request . ' requestID="' . $this->_constructRequestID($next['qb_action'], $next['ident']) . '">', $xml);
                            $xml = str_replace('<' . $request . '>', '<' . $request . ' requestID="' . $next['quickbooks_queue_id'] . '">', $xml);
                            break;
                        }
                    }
                } elseif ($this->_config['check_valid_requestid']) {
    ...
    */
     ...
     * @return boolean
     */
    protected function _callHook($ticket, $hook, $requestID, $action, $ident, $extra, &$err, $xml = '', $qb_identifiers = [], $hook_data = [])
    {
        $user = '';
        if ($ticket) {
     ...
     * @param array $qb_identifiers
     */
    protected function _handleError($ticket, $errnum, $errmsg, $requestID, $action, $ident, $extra, &$err, $xml = '', $qb_identifiers = [])
    {
    ...
    protected function _calculateProgress($ticket)
    {
        if ($this->_driver->authCheck($ticket)) { // Check the ticket
            ...
            $this->_callHook($ticket, QUICKBOOKS_HANDLERS_HOOK_PERCENT, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function receiveResponseXML($obj)
    {
        ...
        if ($this->_driver->authCheck($obj->ticket)) { // Check the ticket
            ...
            $hookerr = '';
            $this->_callHook($obj->ticket, QUICKBOOKS_HANDLERS_HOOK_RECEIVERESPONSEXML, null, null, null, null, $hookerr, null, [], $hookdata);
            ...
            if (strlen($obj->message) or
                ...
                if ($current) {
                    ...
                    */
                    ...
                    if ($obj->message) {
                        $errmsg = $obj->message;
                    } elseif ($status = $this->_extractStatusMessage($obj->response)) {
                        $errmsg = $status;
                    }
    ...
    public function connectionError($obj)
    {
        ...
        if ($this->_driver->authCheck($obj->ticket)) {
            ...
            $hookerr = '';
            $this->_callHook($obj->ticket, QUICKBOOKS_HANDLERS_HOOK_CONNECTIONERROR, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function getLastError($obj)
    {
        ...
        if ($this->_driver->authCheck($obj->ticket)) {
            ...
            $hookerr = '';
            $this->_callHook($obj->ticket, QUICKBOOKS_HANDLERS_HOOK_GETLASTERROR, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function closeConnection($obj)
    {
        ...
        if ($this->_driver->authCheck($obj->ticket)) {
            ...
            $hookerr = '';
            $this->_callHook($obj->ticket, QUICKBOOKS_HANDLERS_HOOK_CLOSECONNECTION, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function serverVersion($obj)
    {
        ...
        $hookdata = [  ];
        $hookerr = '';
        $this->_callHook(null, QUICKBOOKS_HANDLERS_HOOK_SERVERVERSION, null, null, null, null, $hookerr, null, [], $hookdata);
    ...
    public function clientVersion($obj)
    {
        ...
        $hookdata = [];
        $hookerr = '';
        $this->_callHook(null, QUICKBOOKS_HANDLERS_HOOK_CLIENTVERSION, null, null, null, null, $hookerr, null, [], $hookdata);
