...
$map = [
    ...
    QUICKBOOKS_IMPORT_PURCHASEORDER => [ '_quickbooks_purchaseorder_import_request', '_quickbooks_purchaseorder_import_response' ],
    QUICKBOOKS_IMPORT_INVOICE => [ '_quickbooks_invoice_import_request', '_quickbooks_invoice_import_response' ],
    QUICKBOOKS_IMPORT_CUSTOMER => [ '_quickbooks_customer_import_request', '_quickbooks_customer_import_response' ],
    QUICKBOOKS_IMPORT_SALESORDER => [ '_quickbooks_salesorder_import_request', '_quickbooks_salesorder_import_response' ],
    QUICKBOOKS_IMPORT_ITEM => [ '_quickbooks_item_import_request', '_quickbooks_item_import_response' ],
    ];
...
$handler_options = [		// See the comments in the QuickBooks/Server/Handlers.php file
    ...
    if ($Doc = $Parser->parse($errnum, $errmsg)) {
        ...
        foreach ($List->children() as $quickBooksXMLNode) {
            ...
            $look_for = [
                ...
                'PurchaseCost' => [ 'SalesOrPurchase Price', 'SalesAndPurchase PurchaseCost', 'PurchaseCost' ],
                'PurchaseDesc' => [ 'SalesOrPurchase Desc', 'SalesAndPurchase PurchaseDesc', 'PurchaseDesc' ],
...
function _quickbooks_error_e500_notfound($requestID, $user, $action, $ID, $extra, &$err, $xml, $errnum, $errmsg)
{
    ...
    return $action == QUICKBOOKS_IMPORT_PURCHASEORDER;
}
